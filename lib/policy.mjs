...
export function validatePolicy(rawPolicy) {
  ...
  rejectUnknown(scan, new Set(["path", "exclude", "max_file_bytes", "include_gas"]), "scan");
  ...
  const includeGas = scan.include_gas === undefined ? false : scan.include_gas;
  if (typeof includeGas !== "boolean") throw new Error("scan.include_gas must be boolean");
...
export function loadPolicy(repositoryRoot, requestedPath = ".shipproof.yml") {
  const path = resolveRepositoryPath(repositoryRoot, requestedPath, "file");
...
export function buildPolicyGates(repositoryRoot, policy) {
  ...
  if (policy.scan.include_gas) scanArguments.push("--include-gas");
