...
function parseScalar(value, lineNumber) {
  ...
  if (/^-?0\d+$/.test(value)) {
    throw new Error(`policy line ${lineNumber}: numbers with leading zeros are unsupported; quote the value if it is a string`);
  }
...
function parseYamlBlock(tokens, startIndex, indent, depth) {
  ...
  while (index < tokens.length && tokens[index].indent === indent) {
    ...
    if (sequence) {
      ...
      if (/^[A-Za-z_][A-Za-z0-9_-]*:(\s|$)/.test(scalar)) {
        throw new Error(
          `policy line ${token.lineNumber}: sequence items must be plain scalars; mappings inside lists are unsupported`,
        );
      }
...
export function parsePolicyText(source) {
  ...
  if (trimmed.startsWith("{")) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`invalid JSON policy: ${message}`);
    }
...
export function validatePolicy(rawPolicy) {
  ...
  if (!Number.isSafeInteger(maxFileBytes) || maxFileBytes < 1024 || maxFileBytes > 100_000_000) {
    throw new Error("scan.max_file_bytes must be an integer from 1024 through 100000000");
  }
