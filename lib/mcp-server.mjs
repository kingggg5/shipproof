...
function resolveMcpSettings() {
  return {
    ...
    cacheMs: resolveNonNegativeIntEnv("SHIPPROOF_MCP_CACHE_MS", 0, 3_600_000),
  };
}

let activeMcpSettings = null;

function mcpSettings() {
  return activeMcpSettings || resolveMcpSettings();
}
...
async function runPythonJsonCached(command, argumentsList, abortSignal) {
  const { cacheMs } = mcpSettings();
  if (cacheMs <= 0 || command !== "scan") {
  ...
  scanResultCache.set(key, { report, expiresAt: Date.now() + cacheMs });
...
function runPythonProcess(command, argumentsList, { abortSignal, stdin } = {}) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const timeout = setTimeout(() => {
      child.kill();
      finish(new Error(`tool call exceeded the ${timeoutMs / 1000} second limit`));
    }, timeoutMs);
...
export async function startMcpServer() {
  activeMcpSettings = resolveMcpSettings();
  let McpServer;
