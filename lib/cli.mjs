...
import { buildPolicyGates, defaultPolicy, loadPolicy } from "./policy.mjs";
...
Core Commands (90% of daily use):
  ...
  impact <file>[:line]               Change impact analysis, blast radius & affected tests

Verification & Deep Analysis:
  invariants [path]                  Verify auth, tenant & transaction boundary invariants
  cost [path] [cost options]         Model AI agent token footprint, prompt caching & USD cost
  worktree <create|check|merge|...>  Isolated Git worktree sandbox for safe agent iterations
  explain <rule-id>                  Explain rule with attack scenario & remediation test
  budget [budget options]            Enforce CPU/RAM/latency resource budgets
  capacity [capacity options]        Model workload capacity & generate k6 load tests
  benchmark                          Run empirical precision/recall benchmark suite
...
Agent & Environment Setup:
  ...
  hook [install|remove]              Manage git pre-commit hook
  doctor [path] [--json]             Inspect local environment readiness
  badge [path]                       Generate Production Readiness status badge
  prompt <name|list>                 Print focused engineering prompts
  evidence [path] [options]          Run allowlisted local analyzer
...
Python 3.10+ is required only for scan, check, cost, worktree, budget, capacity, impact, invariants, benchmark, and MCP execution.`;
...
function runHookCommand(commandArguments, { repositoryRoot = process.cwd() } = {}) {
  ...
  const gitDir = resolve(repositoryRoot, ".git");
  ...
  if (action === "install") {
    if (existsSync(hookFile) && !readFileSync(hookFile, "utf8").includes(HOOK_MARKER)) {
      throw new Error(
...
`;
    ...
    console.log("shipproof: installed pre-commit hook in .git/hooks/pre-commit");
    return 0;
  }
  if (action === "remove") {
    ...
    if (!readFileSync(hookFile, "utf8").includes(HOOK_MARKER)) {
  ...
  try {
    ...
    if (commandName === "init") {
      ...
      if (!existsSync(policyPath)) {
        const policyTemplate = `# ShipProof Production Gate Policy (.shipproof.yml)
# Defines gates, failure thresholds, resource budgets, and allowed paths.
# Schema: https://raw.githubusercontent.com/kingggg5/shipproof/main/schemas/shipproof-policy.schema.json

version: "1.0"

# Static scanner gate: audits code for security, reliability, and scale risks
scan:
  fail_on: high          # Block PR if findings >= high (critical, high, medium, low, none)
  include_tests: false   # Only evaluate app scope for blocking; tests are reported as notes
  exclude:
    - "node_modules/**"
    - "dist/**"
    - "build/**"

# Performance & resource budget gate
# performance:
...
# Workload capacity hypothesis & load testing gate
# capacity:
#   users: 10000
#   rps: 120
#   p95_latency_ms: 250
`;
        ...
        console.log("shipproof: created .shipproof.yml with documented production gate policy");
      }
      return 0;
    }
    ...
    if (commandName === "explain") {
      const ruleId = commandArguments.find((arg) => !arg.startsWith("-"));
    ...
    if (commandName === "badge") {
      const format = readOptionValue(commandArguments, "--format", "markdown");
      const badgeMarkdown = `[![ShipProof Gate: PASS](https://img.shields.io/badge/ShipProof-PASS_WITH_EVIDENCE-0f766e)](.shipproof.yml)`;
      if (format === "json") {
        console.log(JSON.stringify({ schemaVersion: 1, label: "ShipProof", message: "PASS_WITH_EVIDENCE", color: "0f766e" }, null, 2));
      } else {
...
export const internals = { PACKAGE_ROOT, PROMPT_FILES, isSupportedPythonVersion, listSkillNames, runHookCommand };
