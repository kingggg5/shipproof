...
Usage:
  ...
  (Running \`shipproof\` with no command scans the current directory.)

Commands:
  check [path] [--config <file>]     Scan and gate (works without .shipproof.yml)
  scan [path] [scanner options]      Run the local repository scanner
  explain <rule-id>                  Explain a rule in detail (e.g. explain SP108)
...
Python 3.10+ is required only for scan, check, budget, capacity, and MCP tool execution.`;
...
function runDoctorCommand(rootPath, useJsonOutput) {
  ...
  if (useJsonOutput) console.log(JSON.stringify(report, null, 2));
    ...
    if (!pythonRuntime) {
      console.log("\n  -> To enable all static analysis and capacity gates, install Python 3.10+:");
      if (process.platform === "darwin") console.log("     macOS: brew install python");
      else if (process.platform === "win32") console.log("     Windows: winget install Python.Python.3.12");
