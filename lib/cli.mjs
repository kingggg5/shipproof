...
function runHookCommand(commandArguments, { repositoryRoot = process.cwd() } = {}) {
  ...
  if (action === "install") {
    ...
    writeFileSync(hookFile, hookContent, { encoding: "utf8", mode: 0o755 });
...
export function runCli(argumentsList) {
  ...
  try {
    ...
    if (commandName === "explain") {
      if (!commandArguments[0]) throw new Error("usage: shipproof explain <rule-id>");
      return runPythonCommand(
        PYTHON_COMMANDS.get("scan"),
        ["--explain", commandArguments[0]],
      );
    }
    if (commandName === "hook") {
