...
function resolvePositiveIntEnv(name, fallback, minimum = 1) {
...
const GATE_TIMEOUT_MS = resolvePositiveIntEnv("SHIPPROOF_GATE_TIMEOUT_MS", 60_000, 1_000);
const GATE_MAX_BUFFER_BYTES = resolvePositiveIntEnv("SHIPPROOF_MAX_BUFFER_BYTES", 16_000_000, 65_536);

function describeGateSpawnError(scriptPath, error) {
  if (error?.code === "ETIMEDOUT" || /timed?\s?out/i.test(String(error?.message || ""))) {
    return `policy gate \`${scriptPath}\` exceeded the ${GATE_TIMEOUT_MS} ms timeout; set SHIPPROOF_GATE_TIMEOUT_MS to adjust`;
  }
  if (/maxBuffer/i.test(String(error?.message || ""))) {
    return `policy gate \`${scriptPath}\` exceeded the ${GATE_MAX_BUFFER_BYTES} byte output limit; set SHIPPROOF_MAX_BUFFER_BYTES to adjust`;
  }
...
function runPythonJsonCommand(scriptPath, commandArguments, spawnImpl = spawnSync) {
  ...
  if (processResult.error) throw new Error(describeGateSpawnError(scriptPath, processResult.error));
  if (processResult.status === null || processResult.signal) {
    throw new Error(
      `policy gate \`${scriptPath}\` was terminated (signal ${processResult.signal || "unknown"}); `
        + `it likely exceeded the ${GATE_TIMEOUT_MS} ms timeout`,
    );
  }
  const status = processResult.status;
  if (status > 1) {
