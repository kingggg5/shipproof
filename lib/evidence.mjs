...
function executableAvailable(command, environment, versionArguments = ["--version"]) {
  ...
  if ((isAbsolute(command) || command.includes(sep)) && existsSync(command)) return true;
...
export function discoverEvidenceAdapters(inputPath = ".") {
  ...
  return Object.entries(ADAPTERS).map(([name, adapter]) => {
    ...
    return {
      ...
      available: Boolean(
        invocation
          && executableAvailable(invocation.command, environment, adapter.versionArguments),
      ),
...
export const internals = { ADAPTERS, classifyDiagnostics };
