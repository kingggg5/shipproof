...
from check_budget import evaluate_resource_budget, render_markdown_report  # noqa: E402


class BudgetTests(unittest.TestCase):
    def test_lower_is_better_within_budget(self):
        payload = evaluate_resource_budget(
            {"metrics": {"rss_mb": 100}},
            {"metrics": {"rss_mb": 104}},
            {
                "metrics": {
                    "rss_mb": {"direction": "lower", "max_regression_percent": 5, "max": 110}
                }
            },
        )
        self.assertTrue(payload["passed"])
        self.assertIn("PASS", render_markdown_report(payload))
    ...
    def test_missing_metric_fails_closed(self):
        with self.assertRaisesRegex(ValueError, "missing"):
            evaluate_resource_budget({"rss_mb": 100}, {}, {"rss_mb": {"max": 120}})

    def test_invalid_rule_fails_closed(self):
        with self.assertRaisesRegex(ValueError, "direction"):
            evaluate_resource_budget(
                {"rss_mb": 100}, {"rss_mb": 101}, {"rss_mb": {"direction": "sideways", "max": 120}}
            )
    ...
    def test_cli_execution_with_files(self):
        ...
        with (
            patch("check_budget.Path.read_text", side_effect=payloads),
            contextlib.redirect_stdout(io.StringIO()),
        ):
            code = main(
                ["--baseline", "base.json", "--current", "curr.json", "--budget", "budg.json"]
            )
    ...
    def test_multiple_stdin_inputs_fail_closed(self):
        ...
        with contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(
                main(["--baseline", "-", "--current", "-", "--budget", "budget.json"]), 2
            )
