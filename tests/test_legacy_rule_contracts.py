...
class LegacyPatternContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.index = json.loads((CONTRACT_DIR / "pattern-index.json").read_text(encoding="utf-8"))
        cls.manifests = []
        ...
        cls.rules = {
            rule.rule_id: rule
            for rule in RULES
            if 101 <= rule_number(rule.rule_id) <= 650 and rule.rule_id not in FILE_LEVEL_RULE_IDS
        }

    def test_index_hashes_counts_and_engine_partitions_are_exact(self) -> None:
        ...
        for rule_id, entry in self.entries.items():
            for case in entry["cases"]["positive"]:
                ...
                with self.subTest(rule_id=rule_id, path=case["path"]):
                    self.assertEqual(len(matches), 1, source_for(case)[:160])
                    finding = matches[0]
            ...
            for polarity in ("negative", "adversarial"):
                for case in entry["cases"][polarity]:
                    with self.subTest(rule_id=rule_id, polarity=polarity):
                        ...
                        self.assertEqual(findings_for(entry, case), [], source_for(case)[:160])

    def test_builder_is_deterministic_and_current(self) -> None:
        completed = subprocess.run(
            [sys.executable, "scripts/build_legacy_pattern_contracts.py", "--check"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
