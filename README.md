...
ShipProof applies the same review contract regardless of who wrote the code. Its executable scanner currently contains **605 deterministic rules** for locally observable security, correctness, scale, performance, configuration, and supply-chain risks. The default path is read-only, offline, and dependency-free beyond Node.js and the Python standard library.

| Property | Current contract |
| :--- | :--- |
| Current release | `v0.7.0` public beta |
| Runtime | Node.js 20+; Python 3.10+ for scanner-backed commands |
| Executable rules | 605 (`SP001`–`SP665`, with deliberate reserved gaps) |
...
**605 deterministic executable rules** (`SP001`–`SP665`, with deliberate reserved gaps) across security, correctness, scale, performance, configuration, and supply-chain risks. Findings carry an evidence `proof_level`: `L0` pattern match, `L1` structural/AST/artifact evidence, and `L2` interprocedural taint flows (`--cross-file`; Python plus JavaScript/TypeScript route-to-sink chains since v0.8).
...
| [Executable rule table](docs/rules.md#detection-rules-reference) | 605 reviewed detectors | Emits versioned findings |
